...
```
   ...
   └────────────┘  └──────────┘  └──────────────┘
          ...
          └───────────────┼───────────────┘
                          ▼
                ┌───────────────────┐
                │ Weighted RRF 融合  │  (向量0.5/BM250.3/图谱0.2)
                └───────────────────┘
                          │
                          ▼
                ┌───────────────────┐
...
| 方法 | 关键词命中率 | 拒答正确率 | 平均延迟 |
|---|---|---|---|
| 向量 + BM25 | ... | ... | ... |
| + 知识图谱 | ... | ... | ... |
| + Rerank（全流程） | ... | ... | ... |

测试集位于 `data/test/eval_questions.json`，包含 12 道领域问题（事实型、关系型、概述型、拒答型），可直接扩展。

> 拒答型题目用于检验系统对语料外问题的抗幻觉能力。
